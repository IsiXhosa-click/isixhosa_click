...
impl WriterActor {
    ...
    fn add_word(writer: &mut IndexWriter, schema_info: &SchemaInfo, doc: WordDocument) {
        let stemmed = if doc.part_of_speech == Some(PartOfSpeech::Noun) {
...
impl WordHitExt for WordHit {
    fn try_deserialize(schema_info: &SchemaInfo, doc: Document) -> Result<WordHit> {
        ...
        Ok(WordHit {
            ...
            part_of_speech: get_with_sentinel(&doc, schema_info.part_of_speech).map(SerOnlyDisplay),
...
impl From<WordDocument> for WordHit {
    fn from(d: WordDocument) -> Self {
        WordHit {
            ...
            english: d.english,
            xhosa: d.xhosa,
            part_of_speech: d.part_of_speech.map(SerOnlyDisplay),
